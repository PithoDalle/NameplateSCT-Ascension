...
function NameplateSCT:DamageEvent(guid, spellName, amount, overkill, school, crit, spellId, heals)
	...
	elseif (self.db.global.truncate and amount >= 10000) then
		...
		if (self.db.global.truncateLetter) then text = text .. "k" end
	elseif (self.db.global.truncate and amount >= 1000) then
		text = format("%.1f", amount / 1000)
		if (self.db.global.truncateLetter) then text = text .. "k" end
	else
		if (self.db.global.commaSeperate) then text = commaSeperate(amount) else text = tostring(amount) end
	end
	...
	if (self.db.global.sizing.smallHits or self.db.global.sizing.smallHitsHide) and playerGUID ~= guid then
		...
		if ((not crit and amount < SMALL_HIT_MULTIPIER * runningAverageDamageEvents) or (crit and amount / 2 < SMALL_HIT_MULTIPIER * runningAverageDamageEvents)) then
			if (self.db.global.sizing.smallHitsHide) then return else size = size * self.db.global.sizing.smallHitsScale end
		end
	end

	if (self.db.global.sizing.crits and crit) and playerGUID ~= guid then
		if (autoattack and not self.db.global.sizing.autoattackcritsizing) then pow = false else size = size * self.db.global.sizing.critsScale end
	end
...
function NameplateSCT:MissEvent(guid, spellName, missType, spellId)
	...
	local isTarget = (UnitGUID("target") == guid)
	if playerGUID ~= guid then animation = self.db.global.animations.miss color = self.db.global.defaultColor else animation = self.db.global.animationsPersonal.miss color = self.db.global.defaultColorPersonal end
	if (animation == "disabled") then return end
	if (self.db.global.useOffTarget and not isTarget and playerGUID ~= guid) then size = self.db.global.offTargetFormatting.size alpha = self.db.global.offTargetFormatting.alpha else size = self.db.global.formatting.size alpha = self.db.global.formatting.alpha end
	if self.db.global.sizing.miss and playerGUID ~= guid then size = size * self.db.global.sizing.missScale end
	pow = true
...
function NameplateSCT:ColorText(startingText, guid, playerGUID, school, spellName, heals)
	...
	if guid ~= playerGUID then
		if self.db.global.damageColor and (spellName == AutoAttack or spellName == AutoShot) and DAMAGE_TYPE_COLORS[spellName] then finalText = "\124cff" .. DAMAGE_TYPE_COLORS[spellName] .. startingText .. "\124r"
		elseif self.db.global.damageColor and school and DAMAGE_TYPE_COLORS[school] then finalText = "\124cff" .. DAMAGE_TYPE_COLORS[school] .. startingText .. "\124r"
		else finalText = "\124cff" .. self.db.global.defaultColor .. startingText .. "\124r" end
	else
		if self.db.global.damageColorPersonal and school and DAMAGE_TYPE_COLORS[school] then finalText = "\124cff" .. DAMAGE_TYPE_COLORS[school] .. startingText .. "\124r"
		elseif self.db.global.damageColorPersonal and spellName == AutoAttack and DAMAGE_TYPE_COLORS[spellName] then finalText = "\124cff" .. DAMAGE_TYPE_COLORS[spellName] .. startingText .. "\124r"
		elseif heals and not self.db.global.damageColorPersonal then finalText = "\124cff4dff4d" .. startingText .. "\124r"
		else finalText = "\124cff" .. self.db.global.defaultColorPersonal .. startingText .. "\124r" end
	end
	return finalText
end
...
function NameplateSCT:GetNameplate(guid)
	...
	-- 1. Loads first if ascension uses retail nameplate naming
	if C_NamePlate and C_NamePlate.GetNamePlateForUnit and UnitGUID("target") == guid then
		local cp = C_NamePlate.GetNamePlateForUnit("target")
	...
	if not plate and UnitExists("target") and not UnitIsUnit("target", "player") and UnitGUID("target") == guid then 
		...
		-- 2. ASCENSION TARGET HACK: In case LibNameplates fails, we'll find it manually
		if not plate then
			for _, frame in ipairs({WorldFrame:GetChildren()}) do
				local name = frame:GetName()
				if name and string.find(name, "NamePlate") and frame:IsShown() and frame:GetAlpha() == 1 then
					plate = frame
					break
				end
			end
...
function NameplateSCT:DisplayText(guid, text, size, alpha, animation, spellId, pow, spellName)
    ...
    local anchor = nameplate or "player" 
    ...
    if self.db.global.textShadow then fontString:SetShadowOffset(1, -1) else fontString:SetShadowOffset(0, 0) end
    fontString.startHeight = fontString:GetStringHeight()
    fontString.pow = pow
    if (fontString.startHeight <= 0) then fontString.startHeight = 5 end
    fontString.guid = guid
    local _, _, texture = GetSpellInfo(spellId or spellName)
    if not texture and spellName then _, _, texture = GetSpellInfo(spellName) end
    if self.db.global.showIcon and texture then
        local icon = fontString.icon
        icon:Show()
        ...
        icon:SetAlpha(alpha)
    elseif fontString.icon then fontString.icon:Hide() end
    self:Animate(fontString, anchor, self.db.global.animations.animationspeed, animation)
end
...
function NameplateSCT:DisplayTextOverkill(guid, text, size, alpha, animation, spellId, pow, spellName)
	...
	if self.db.global.showIcon and texture then
		...
		icon:SetAlpha(alpha)
	elseif fontString.icon then fontString.icon:Hide() end
	self:Animate(fontString, "player", self.db.global.animations.animationspeed, animation)
end
